...
namespace SimulatedPuzzle
{
    internal class Program
    {
        static async Task Main(string[] args)
        {
            ...
            var staticId = Guid.NewGuid().ToString();
            while (true)
            {
                ...
                switch (key.Key)
                {
                    ...
                    case ConsoleKey.I:
                        var idMessage = new ServiceBusMessage("{}");
                        idMessage.ApplicationProperties["MessageType"] = "PuzzleEvent";
                        idMessage.MessageId = staticId;

                        await sender.SendMessageAsync(idMessage);
                        Console.WriteLine("Message sent\n");
                        break;
